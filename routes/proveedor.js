...
router.post("/proveedor", (req, res) => {
  ...
  if (!validaCorreo && !validaDocumento) {
    console.log(nombre);
    pool.query(
      consultas.INSERTPROVEEDOR,
      [nombre, correo, direccion, idTipoDocumento, numeroDocumento],
      (err, rows, fields) => {
        if (!err) {
...
router.post("/deactivate", (req, res) => {
  ...
  pool.query(consultas.DESPROVEEDOR, [idProveedor], (err, rows, fields) => {
    if (!err) {
...
router.put("/update", (req, res) => {
  const { id, nombre, correo, direccion, idTipoDocumento, numeroDocumento } =
    req.body;

  pool.query(
    consultas.UPDATEPROVEEDOR,
    [nombre, correo, direccion, idTipoDocumento, numeroDocumento, id],
    (err, rows, fields) => {
      if (!err) {
