...
router.post(
  "/tipo",
  body("tipoDoc").not().isEmpty().trim().escape(),
  (req, res) => {
    ...
    pool.query(consultas.INSERTTIPODOC, tipoDoc, (err, rows, fields) => {
      if (err) throw err;
      else {
...
router.put("/desactivar", (req, res) => {
  ...
  pool.query(consultas.DESACTIVARTIPODOC, idTipoDoc, (err, rows, fields) => {
    if (err) throw err;
    else {
      res.json({ status: 200, data: [] });
    }
